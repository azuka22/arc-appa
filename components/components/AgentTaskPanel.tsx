...
export default function AgentTaskPanel() {
  ...
  return (
    <div className={styles.panel}>
      ...
      <button className={styles.btn} onClick={run} disabled={loading || !input.trim()}>
        {loading ? "Running agent..." : "Run agent →"}
      </button>
      {logs.length > 0 && (
        <div className={styles.log}>
          {logs.map((l, i) => (
            <div key={i} className={styles[`log-${l.type}`]}>{l.text}</div>
          ))}
        </div>
      )}
      {result && (
        <div className={styles.result}>
          <div className={styles.resultRow}><span>Wallet</span><code>{result.walletAddress}</code></div>
          <div className={styles.resultRow}><span>Service</span><code>{result.serviceUsed}</code></div>
          <div className={styles.resultRow}><span>Paid</span><code>${result.amountPaid} USDC</code></div>
          <div className={styles.resultRow}><span>Output</span><span>{result.output}</span></div>
        </div>
      )}
    </div>
  );
